...
@section('content')
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3>Tasks <a href="{{ route('addProject') }}"><button class="btn btn-primary">Add Task</button></a></h3>
          </div>
          <div class="col-sm-6">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped mt-3">
                  ...
                  <tr>
                    ...
                    <td class="project-state"><span class="badge badge-success">{{$task->status ?? ''}}</span></td>
                    <td class="project-actions">
                      <a class="btn btn-primary btn-sm" href="{{route('viewProject', ['slug' => $project->slug])}}">
                        <i class="fas fa-folder"></i> View
                      </a>
                      <a class="btn btn-info btn-sm" href="{{ route('editTask', ['id' => $task->id]) }}">
                          <i class="fas fa-pencil-alt"></i> Edit
                      </a>
                      <form method="POST" action="{{ route('deleteTask', $task->id) }}" style="display: inline-block;">
                        @csrf
