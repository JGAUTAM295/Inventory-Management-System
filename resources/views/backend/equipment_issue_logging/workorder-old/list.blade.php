...
@section('content')
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3>Work Orders  @if(in_array('work_order.create', $groupsWithRoles))<a href="{{ route('work_order.create') }}"><button class="btn btn-primary">Add Work Order</button></a>@endif
            @if(in_array('equipment_issues.index', $groupsWithRoles))<a href="{{ route('equipment_issues.index') }}"><button class="btn btn-primary">Back</button></a>@endif</h3>
          </div>
          <div class="col-sm-6">
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped mt-3">
                  ...
                  <tr>
                    <td>{{ $key+1 }}</td>
                    <td>{{ ucwords($work_order->jobperformed->name) ?? ''}}</td>
                    <td>{{ date('d M, Y h:i A', strtotime($work_order->start_date)) ?? ''}}</td>
                    <td>{{ date('d M, Y h:i A', strtotime($work_order->end_date)) ?? ''}}</td>
                    <td>{!! Str::limit(ucwords($work_order->remarks), 100, ' ...') !!}
                    <td class="project-actions">
                      @if(in_array('equipment_issues_order.edit', $groupsWithRoles))
                      <a class="btn btn-info btn-sm" href="{{ route('equipment_issues_order.edit', ['id'=>$id,'eilid'=>$work_order->id]) }}">
                        <i class="fas fa-pencil-alt"></i>  Edit
                      </a>
                      @endif
                      @if(in_array('equipment_issues_order.destroy', $groupsWithRoles))
                      <form method="POST" action="{{ route('equipment_issues_order.destroy', ['id'=>$id,'eilid'=>$work_order->id] ) }}" style="display: inline-block;">
                        @csrf
