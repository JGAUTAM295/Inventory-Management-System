...
@section('content')
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3>Menu Item @if(in_array('menus.create', $groupsWithRoles))<a href="{{ route('menus.create') }}"><button class="btn btn-primary">Add Menu Item</button></a>@endif</h3>
          </div>
          <div class="col-sm-6">
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped mt-3">
                  ...
                  <tr>
                    <td>{{ucwords($menu->menu_item) ?? ''}}</td>
                    <td class="project-state">@if(!empty($menu->parent_menu))<span class="badge badge-success"> Yes </span> @else <span class="badge badge-danger"> No </span>@endif</td>
                    <td>{{$menu->menu_order ?? ''}}</td>
                    <td class="project-state"> @if($menu->status == '1') <span class="badge badge-success">Active</span>@elseif($menu->status == '2') <span class="badge badge-danger">Deactive</span>@endif</td>
                    <td class="project-actions">
                      @if(in_array('menus.edit', $groupsWithRoles))
                      <a class="btn btn-info btn-sm" href="{{ route('menus.edit', $menu->id) }}">
                          <i class="fas fa-pencil-alt"></i>
                          Edit
                      </a>
                      @endif
                      @if(in_array('menus.destroy', $groupsWithRoles))
                      <form method="POST" action="{{ route('menus.destroy', $menu->id) }}" style="display: inline-block;">
                        @csrf
